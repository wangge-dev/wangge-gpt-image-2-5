...
def main():
    ...
    lines += ['## 一款商品连续出素材', '', '[一张商品图，三种电商用途 · EvoLink来源案例](evolink-product-story.md)', '', '建议顺序：EC01 主图 → EC02 场景 → EC04 细节 → EC11 卖点 → EC13 尺寸 → EC05 活动。每次重新附商品原图，使用同一套真实文案和配色。', '', '### 按品类直接执行', '', '- [美妆个护：6步完整提示词](kits/beauty.md)', '- [服饰：6步完整提示词](kits/apparel.md)', '- [食品：6步完整提示词](kits/food.md)', '', '三套任务包均给出素材清单、逐步完整提示词、验收点和局部返工指令。其他品类可使用[一品多图组合](product-kit.md)。已有的[精准改字](../experiments/E003-copy-edit.md)、[多轮返工](../experiments/E005-revision-chain.md)与[跨境适配](../experiments/E006-localization.md)也可以直接使用。', '', '## 持续补充', '', '持续补充品类专用提示词、场景变体与编辑技巧。欢迎在[贡献说明](../CONTRIBUTING.md)中提交新需求。']
