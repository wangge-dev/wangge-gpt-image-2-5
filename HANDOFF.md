...
- 492条基础提示词：472张 `data/library.json` 卡、11条 `cases/`、9条 `experiments/`。
- 132个完整变体。
- 48套电商场景；EC01–EC48有填写完成的直接示例；另有55条精选创意；T366–T369补遥控器电池仓、真无线耳机、眼镜与气垫粉盒的真实归属关系；阶段T推进到492/500。
...
- 最近已发布内容提交：`1b68d21`（阶段T第3批；阶段S收口与阶段T前两批均已随提交链发布）。
- 当前本地 `main` 与远端跟踪分支一致；阶段T继续推进。
...
- `python scripts/check_links.py`：2992个本地Markdown链接通过。
- 472个JSON卡，无重复ID，生成卡齐全。
- 总条目492、变体132；八套任务包48步、48个文本块、无双花括号变量。
