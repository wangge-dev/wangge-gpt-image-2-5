...
建设一个独立、持续更新、真正面向GPT Image 2.5的中文提示词库。近期优先做到：电商用户提供商品素材后，能找到接下来需要制作的一整套素材，按顺序复制提示词，并知道每一步如何验收和返工。先堆提示词和跑通流程，统一生图测试放在后面。新的扩建配比为50%高频电商成片、25%编辑与一致性、15%创意商业视觉、10%结构核对；重要的是视觉结构。

## 当前完成状态

- 532条基础提示词：512张 `data/library.json` 卡、11条 `cases/`、9条 `experiments/`。
- 132个完整变体。
- 48套电商场景；EC01–EC48有填写完成的直接示例；另有55条精选创意；T406–T409补光圈叶片联动、风绳分段守长、双搅拌棒与专属空孔、标签离纸转贴；阶段V进度532/540。
...
- 最近已发布内容提交：`bddda7e`（阶段U已收口，阶段V进度528/540）。
- 当前本地 `main` 与远端跟踪分支一致；阶段V继续只堆提示词，不生成图片。
...
- `python scripts/check_links.py`：3192个本地Markdown链接通过。
- 512个JSON卡，无重复ID，生成卡齐全。
- 总条目532、变体132；八套任务包48步、48个文本块、无双花括号变量。
