...
按[持续扩库路线](roadmap-2026-09-13-prompt-expansion.md)推进。每个有效批次默认完成3–5条，优先电商实用缺口、中文排版、参考图编辑，再补有独特商业机制的创意。完成正文、输入材料及职责、变量示例、无占位符直接示例、完整变体和局部修改指令；未生图不阻止收录，资料缺失进入待补充清单。没有足够合格候选时做少量高价值修订，不凑数。
...
记录来源访问成功与失败、候选淘汰原因、实际新增或修订、检查与同步结果。始终保持 Image 2.5 中文提示词库定位，不转向泛模型提示词、生成平台或文章项目；不自动收费生图。完成一批、事实修订、失败或需用户输入时通知。
```
